...
and annualised Alpha $\approx +38.0\%$ against Nifty 500, an Information Ratio of 2.65, a
Sortino Ratio of 3.96, and a Calmar Ratio of 2.08. Beta below 1 shows the outperformance is not
...
Outperformance (CAGR, pp) & -- & +36.79 & +38.79 \\
\bottomrule
\end{tabular}
...
in that same blind test ($+8.8\%$). This is encouraging evidence the fix works, but it is a
...
while already trading below its own 200-day average on rebalance day; such a name is simply not
bought (its allocation is zeroed and, if trend re-entry is enabled, it is queued to enter once
price actually reclaims the average). Every trading decision for a name -- the initial trim or
buy, and any redeployment top-up -- is netted into a single signed order before anything
executes, so at most one buy or one sell is ever placed per name per rebalance. Names being
trimmed this quarter are also excluded from redeployment entirely: this is a deliberate design
rule, tested and confirmed to improve CAGR, Sharpe and drawdown, not merely a cheaper way to
trade, since redeploying capital into a name the model just decided deserves less weight would
undercut that quarter's own rebalancing signal.
